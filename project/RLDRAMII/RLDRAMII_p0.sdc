...
#      in the RLDRAMII_p0_timing.tcl script.
#    * The helper routines are defined in RLDRAMII_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/RLDRAMII_p0_timing.tcl"
...
set io_standard "$::GLOBAL_RLDRAMII_p0_io_standard CLASS I"
...
set outputDQSpathjitter [expr [get_io_standard_node_delay -dst OUTPUT_DQDQS_JITTER -io_standard $io_standard -parameters [list IO $::GLOBAL_RLDRAMII_p0_io_interface_type] -in_fitter]/1000.0]

# This is the proportion of the DQ-DQS write path jitter that applies to setup
set outputDQSpathjitter_setup_prop [expr [get_io_standard_node_delay -dst OUTPUT_DQDQS_JITTER_DIVISION -io_standard $io_standard -parameters [list IO $::GLOBAL_RLDRAMII_p0_io_interface_type] -in_fitter]/100.0]
...
set half_period [ RLDRAMII_p0_round_3dp [ expr $t(CK) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ RLDRAMII_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]

# Minimum distance between two consecutive edges of CK (positive to negative or viceversa)
set t(QKHQKnH) [ RLDRAMII_p0_round_3dp [ expr $t(QKH) * $t(CK) * $t(CKH)] ]
...
set data_output_min_delay [ RLDRAMII_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ RLDRAMII_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]

# Maximum delay on data input pins
set t(rdu_input_max_delay_external) [expr $t(QKQ_max) + $board(intra_DQS_group_skew) + $board(data_QK_skew) + $ISI(READ_DQ)/2 + $ISI(READ_DQS)/2]
set t(rdu_input_max_delay_internal) [expr $DQSpathjitter*$DQSpathjitter_setup_prop + $SSN(rel_pushout_i)]
set data_input_max_delay [ RLDRAMII_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]

# Minimum delay on data input pins
set t(rdu_input_min_delay_external) [expr -$t(QKQ_min) + $board(intra_DQS_group_skew) - $board(data_QK_skew) + $ISI(READ_DQ)/2 + $ISI(READ_DQS)/2]
set t(rdu_input_min_delay_internal) [expr $t(DCD) + $DQSpathjitter*(1.0-$DQSpathjitter_setup_prop) + $SSN(rel_pullin_i)]
set data_input_min_delay [ RLDRAMII_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ RLDRAMII_p0_round_3dp [ expr - $t(AH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold) ]]

# Maximum delay on address and command paths
set ac_max_delay [ RLDRAMII_p0_round_3dp [ expr $t(AS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup) ]]
...
if { ! [ info exists RLDRAMII_p0_sdc_cache ] } {
	set RLDRAMII_p0_sdc_cache 1
	RLDRAMII_p0_initialize_ddr_db RLDRAMII_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names RLDRAMII_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $RLDRAMII_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_RLDRAMII_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set data_input_min_constraint [ RLDRAMII_p0_round_3dp [ expr -$t(QKHQKnH) + $fpga(tDQS_PSERR) ]]

	# Mimimum delay requirement for hold constraint on read path
	set data_input_max_constraint [ RLDRAMII_p0_round_3dp [ expr - $fpga(tDQS_PSERR) ]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_RLDRAMII_p0_dqs_delay_chain_length"
	...
	# AFI clock
	set local_pll_afi_clk [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_AFI_CLK) ]

	# HR PHY clock
	set local_pll_afi_phy_clk [ RLDRAMII_p0_get_or_add_clock_vseries \
	...
	# Write clock
	set local_pll_write_clk [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_dk_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_WRITE_CLK) ]
		
	# Common clock between CK/DK at the PHY clock tree output
	set local_pll_mem_clk [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_ck_clock \
		-suffix "mem_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_MEM_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_MEM_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_MEM_CLK) ]		

	# A/C clock
	set local_pll_addr_cmd_clk [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_ac_clock \
		-suffix "addr_cmd_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_ADDR_CMD_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_ADDR_CMD_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_ADDR_CMD_CLK) ]

	# NIOS clock
	set local_pll_avl_clock [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_NIOS_CLK) ]
	
	# I/O scan chain clock
	set local_pll_config_clock [ RLDRAMII_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_RLDRAMII_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_RLDRAMII_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_RLDRAMII_p0_pll_phase(PLL_CONFIG_CLK) ]	
	...
	set number_of_clocks_per_dm [ expr $::GLOBAL_RLDRAMII_p0_number_of_d_groups / $::GLOBAL_RLDRAMII_p0_number_of_dm_pins ]
	...
	foreach { ck_pin } $ck_pins {
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -max [RLDRAMII_p0_round_3dp [expr $t(CK) - $t(CKDK_max)]] $dk_pins
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -min [RLDRAMII_p0_round_3dp [expr -$t(CKDK_min)]] $dk_pins		
	...
	set ac_fr_phase_mod_360 [expr round(2*$::GLOBAL_RLDRAMII_p0_pll_phase(3)) % 360]
	set ck_fr_phase_mod_360 [expr round($::GLOBAL_RLDRAMII_p0_pll_phase(1)) % 360]
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [RLDRAMII_p0_get_all_instances_dqs_pins RLDRAMII_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_RLDRAMII_p0_corename}_report_timing.tcl]]"
